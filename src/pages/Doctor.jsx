...
const DESTINOS_CON_CAMA_HOSPITAL = ['OBSERVACION', 'HOSPITALIZACION', 'TERAPIA_INTENSIVA'];
...
export default function Doctor() {
  ...
  const pacientesFiltrados = pacientes
    ...
    .filter(p => !soloMios || (p.atendido_por && p.atendido_por === user?.nombre) || (p.atendido_por && p.atendido_por === user?.email))
  ...
  const cerrarModal = async () => {
    if (!consultaFinalizada && pacienteModal) {
      try {
        await updateDoc(doc(db, 'pacientes', pacienteModal.id), {
          estado: 'espera',
          consulta_inicio: null,
          atendido_por: null,
        });
      } catch (err) {
        console.error('Error al revertir consulta:', err);
      }
    }
    setModalOpen(false);
  ...
  const atenderPaciente = async (paciente) => {
    ...
    setDiagnostico('');
    setObservaciones('');
    setDoctorNombre(user?.nombre || user?.email || '');
    setPasoModal(1);
    setConsultaInicio(Date.now());
    setConsultaFinalizada(false);
    setNuevosVitales({});
    setAlarmasChecklist({});
    setCamaAsignada('');
    ...
    try {
      await updateDoc(doc(db, 'pacientes', paciente.id), {
        ...
        atendido_por: user?.nombre || user?.email || 'Médico',
      });
    } catch (err) {
  ...
  const confirmarAtencion = async () => {
    if (!destino) {
      addToast('Selecciona un destino para el paciente', 'warning', 3000, 'Destino requerido');
      return;
    }
    if (DESTINOS_CON_CAMA.includes(destino) && !camaAsignada.trim()) {
      addToast('Asigna una cama para este destino', 'warning', 3000, 'Cama requerida');
      return;
    }
    ...
    try {
      const update = {
        estado: destino === 'ALTA' ? 'alta' : destino.toLowerCase(),
        ubicacion_actual: destino,
        fecha_atencion: serverTimestamp(),
        consulta_fin: serverTimestamp(),
      };
      ...
      if (destino === 'ALTA') {
        // 🆕 CORREGIDO: Asegurar que origen y cita_id se pasen explícitamente
        const pacienteCompleto = {
          ...pacienteModal,
          diagnostico_cie10: cie10,
          diagnostico_descripcion: diagnostico,
          diagnostico_observaciones: observaciones,
          atendido_por: doctorNombre,
          ubicacion_actual: destino,
          // Forzar origen y cita_id por si vienen undefined del listener
          origen: pacienteModal.origen || 'manual',
          cita_id: pacienteModal.cita_id || null,
        };
        
        console.log('📋 [Doctor] Archivando paciente:', {
          nombre: pacienteCompleto.nombre,
          origen: pacienteCompleto.origen,
          cita_id: pacienteCompleto.cita_id
        });
        
        const archivado = await archivarPaciente(pacienteCompleto);
        if (archivado) {
          addToast(`${pacienteModal.nombre} archivado en historial`, 'success', 3000, '📋 Historial');
          
          // Notificar si venía de cita
          if (pacienteCompleto.origen === 'cita' && pacienteCompleto.cita_id) {
            addToast('Cita marcada como completada ✅', 'success', 3000, '📅 Cita completada');
          }
        }
      }
      ...
      addToast('Error al guardar', 'error', 4000, 'Error');
    } finally {
      setGuardando(false);
    }
  };
    ...
    <div className="page-container">
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
        <div>
          ...
          <p className="page-subtitle">Pacientes en espera y en consulta</p>
        </div>
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <div className="card" style={{ padding: '12px 16px', textAlign: 'center' }}>
            <div className="kpi-value" style={{ fontSize: '22px' }}>{pacientes.length}</div>
          ...
          <div className="modal-box" style={{ maxWidth: '620px' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 24px 0', borderBottom: '1px solid #E5E7EB' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <div>
                  <h2 style={{ fontSize: '18px', fontWeight: 600 }}>{pacienteModal.nombre}</h2>
                  <p style={{ fontSize: '13px', color: '#6B7280' }}>{pacienteModal.edad} años · {pacienteModal.especialidad}</p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#6B7280', fontSize: '13px' }}>
                    <Clock size={14} />
                    <span>{tiempoConsulta} min</span>
                  </div>
                  <button onClick={cerrarModal} className="btn btn-ghost btn-sm" style={{ width: '32px', height: '32px', padding: 0 }}>
                    <X size={18} />
                  </button>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '16px' }}>
                {['Evaluación', 'Diagnóstico', 'Destino'].map((label, idx) => (
                  <button key={idx} onClick={() => setPasoModal(idx + 1)}
                    style={{
                      flex: 1, padding: '8px', borderRadius: '10px', border: 'none',
                      fontSize: '12px', fontWeight: 600, cursor: 'pointer',
                      background: pasoModal === idx + 1 ? '#3B82F6' : pasoModal > idx + 1 ? '#DBEAFE' : '#F3F4F6',
                      color: pasoModal === idx + 1 ? '#FFF' : pasoModal > idx + 1 ? '#3B82F6' : '#9CA3AF',
                    }}>
                    {idx + 1}. {label}{pasoModal > idx + 1 ? ' ✓' : ''}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="modal-body" style={{ padding: '20px 24px' }}>
              {pasoModal === 1 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div className="card" style={{ padding: '16px', background: '#F9FAFB', boxShadow: 'none' }}>
                    <p className="label">Motivo de consulta</p>
                    <p style={{ fontSize: '15px', fontWeight: 500 }}>{pacienteModal.motivo}</p>
                    {pacienteModal.antecedentes && (
                      <>
                        <p className="label" style={{ marginTop: '12px' }}>Antecedentes</p>
                        <p style={{ fontSize: '14px', color: '#6B7280' }}>{pacienteModal.antecedentes}</p>
                      </>
                    )}
                  </div>

                  <div>
                    <p className="label">Actualizar signos vitales</p>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '10px' }}>
                      {[
                        { id: 'ta', label: 'T/A (mmHg)', ph: pacienteModal.vitales?.ta || '120/80' },
                        { id: 'temp', label: 'Temp (°C)', ph: pacienteModal.vitales?.temp || '36.5' },
                        { id: 'fc', label: 'FC (lpm)', ph: pacienteModal.vitales?.fc || '80' },
                        { id: 'spo2', label: 'SpO₂ (%)', ph: pacienteModal.vitales?.spo2 || '98' },
                        { id: 'glu', label: 'Glucosa (mg/dL)', ph: pacienteModal.vitales?.glu || '90' },
                      ].map(v => (
                        <div key={v.id}>
                          ...
                          <input
                            type="text"
                            placeholder={v.ph}
                            value={nuevosVitales[v.id] || ''}
                            onChange={e => setNuevosVitales(prev => ({ ...prev, [v.id]: e.target.value }))}
                  ...
                  {pacienteModal.signosAlarma?.length > 0 && (
                    <div>
                      <p className="label" style={{ color: '#DC2626' }}>⚠️ Verificar signos de alarma</p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {pacienteModal.signosAlarma.map(alarma => (
                          <label key={alarma} style={{
                            display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer',
                            padding: '8px 12px', borderRadius: '8px',
                            background: alarmasChecklist[alarma] ? '#FEF2F2' : '#F9FAFB',
                            border: `1px solid ${alarmasChecklist[alarma] ? '#FECACA' : '#E5E7EB'}`,
                            fontSize: '13px'
                          }}>
                            <input
                              type="checkbox"
                              checked={!!alarmasChecklist[alarma]}
                              onChange={e => setAlarmasChecklist(prev => ({ ...prev, [alarma]: e.target.checked }))}
                  ...
                  <button onClick={() => setPasoModal(2)} className="btn btn-primary" style={{ alignSelf: 'flex-end' }}>
                    Siguiente: Diagnóstico <ArrowRight size={16} />
                  </button>
                </div>
              )}
              ...
              {pasoModal === 2 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div>
                    ...
                    <textarea value={diagnostico} onChange={e => setDiagnostico(e.target.value)} rows={2} placeholder="Describa el diagnóstico clínico..." className="input-modern" />
                  </div>
                  <div>
                    <label className="label">Código CIE-10</label>
                    <input type="text" value={cie10} onChange={e => setCie10(e.target.value.toUpperCase())} placeholder="Ej: J06.9" className="input-modern" style={{ textTransform: 'uppercase' }} maxLength={10} />
                  </div>
                  <div>
                    <label className="label">Observaciones / Tratamiento</label>
                    <textarea value={observaciones} onChange={e => setObservaciones(e.target.value)} rows={3} placeholder="Medicación, indicaciones, seguimiento..." className="input-modern" />
                  </div>
                  <div>
                    <label className="label">Médico que atiende</label>
                    <input type="text" value={doctorNombre} onChange={e => setDoctorNombre(e.target.value)} placeholder="Dr. / Dra." className="input-modern" style={{ backgroundColor: '#F9FAFB' }} />
                  </div>
                  <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                    <button onClick={() => setPasoModal(1)} className="btn btn-secondary">← Volver</button>
                    <button onClick={() => setPasoModal(3)} className="btn btn-primary">Siguiente: Destino <ArrowRight size={16} /></button>
                  </div>
                </div>
              )}
              ...
              {pasoModal === 3 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
                    {DESTINOS.map(d => (
                      <button key={d.key} onClick={() => { setDestino(d.key); if (!DESTINOS_CON_CAMA.includes(d.key)) setCamaAsignada(''); }}
                        style={{
                          padding: '16px', borderRadius: '14px', border: '2px solid',
                          textAlign: 'left', cursor: 'pointer', background: destino === d.key ? '#F9FAFB' : '#FFF',
                          borderColor: destino === d.key ? d.color : '#E5E7EB',
                          display: 'flex', alignItems: 'center', gap: '12px'
                        }}>
                        <span style={{ fontSize: '24px' }}>{d.icon}</span>
                        <div>
                          <div style={{ fontSize: '14px', fontWeight: 600, color: '#1F2937' }}>{d.label}</div>
                          <div style={{ fontSize: '11px', color: '#9CA3AF' }}>{d.desc}</div>
                        </div>
                        {destino === d.key && <CheckCircle size={18} style={{ color: d.color, marginLeft: 'auto' }} />}
                      </button>
                    ))}
                  </div>
                  {DESTINOS_CON_CAMA.includes(destino) && (
                    <div>
                      <label className="label">Asignar cama</label>
                      <input
                        type="text"
                        value={camaAsignada}
                        onChange={e => setCamaAsignada(e.target.value)}
                        placeholder={`Ej: ${destino === 'INTERNAMIENTO' ? 'Cuarto 101' : destino === 'OBSERVACION' ? 'Obs 3' : destino === 'HOSPITALIZACION' ? 'Hab 201' : 'UCI 5'}`}
                        className="input-modern"
                      />
                    </div>
                  )}
                  <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '8px' }}>
                    <button onClick={() => setPasoModal(2)} className="btn btn-secondary">← Volver</button>
                    <button onClick={confirmarAtencion} disabled={!destino || guardando} className="btn btn-primary btn-lg">
