...
export default function Doctor() {
  ...
  const cerrarModal = async () => {
    if (!consultaFinalizada && pacienteModal) {
      try { await updateDoc(doc(db, 'pacientes', pacienteModal.id), { estado: 'espera', consulta_inicio: null, atendido_por: null }); }
      catch (err) { console.error('Error al revertir consulta:', err); }
    }
    setModalOpen(false); setPacienteModal(null); setConsultaFinalizada(false);
  };
  ...
  const atenderPaciente = async (paciente) => {
    ...
    setConsultaFinalizada(false); setNuevosVitales({}); setAlarmasChecklist({}); setCamaAsignada(''); setModalOpen(true);
    try { await updateDoc(doc(db, 'pacientes', paciente.id), { estado: 'en consulta', consulta_inicio: serverTimestamp(), atendido_por: user?.nombre || user?.email || 'Médico' }); }
    catch (err) { console.error('Error al iniciar consulta:', err); }
  };
  ...
  const llamarYAtender = async () => {
    ...
    if (!siguiente) return;
    try {
      await addDoc(collection(db, 'llamados'), { paciente: siguiente.nombre, nivel: siguiente.nivel_prioridad, consultorio: user?.nombre || user?.email || 'Consultorio', timestamp: serverTimestamp(), activo: true });
    ...
    try {
      ...
      if (destino === 'ALTA') {
        ...
        if (archivado) {
          ...
          if (pacienteCompleto.origen === 'cita' && pacienteCompleto.cita_id) addToast('Cita marcada como completada ✅', 'success', 3000, '📅 Cita completada');
        }
      }

      setConsultaFinalizada(true); setModalOpen(false);
      addToast(`Atención de ${pacienteModal.nombre} finalizada`, 'success', 4000, '✅ Completada');
    } catch (err) { console.error('Error al guardar:', err); addToast('Error al guardar', 'error', 4000, 'Error'); }
    ...
    <div className="page-container">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: 'calc(100vh - 380px)', overflowY: 'auto' }}>
        {pacientesFiltrados.length === 0 ? (
          <div className="empty-state"><div className="empty-state-icon">🩺</div><p className="empty-state-text">{pacientes.length === 0 ? 'No hay pacientes' : 'Sin resultados'}</p></div>
        ) : (
          pacientesFiltrados.slice(0, 50).map((p, i) => {
            const minutos = getMinutos(p); const v = p.vitales || {}; const n = Math.min((p.nivel_prioridad || 5), 5);
            const enConsulta = p.estado === 'en consulta'; const esSiguiente = i === 0 && !enConsulta;
      ...
      {pacientesFiltrados.filter(p => p.estado === 'espera').length > 0 && (
        <button onClick={llamarYAtender} className="btn btn-primary btn-lg" style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 40, boxShadow: '0 4px 20px rgba(59,130,246,0.3)' }}>
          ...
          <div className="modal-box" style={{ maxWidth: '620px' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px 24px 0', borderBottom: '1px solid #E5E7EB' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <div><h2 style={{ fontSize: '18px', fontWeight: 600 }}>{pacienteModal.nombre}</h2><p style={{ fontSize: '13px', color: '#6B7280' }}>{pacienteModal.edad} años · {pacienteModal.especialidad}</p></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#6B7280', fontSize: '13px' }}><Clock size={14} /><span>{tiempoConsulta} min</span></div><button onClick={cerrarModal} className="btn btn-ghost btn-sm" style={{ width: '32px', height: '32px', padding: 0 }}><X size={18} /></button></div>
              </div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '16px' }}>
                {['Evaluación', 'Diagnóstico', 'Destino'].map((label, idx) => (
                  <button key={idx} onClick={() => setPasoModal(idx + 1)} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: 'none', fontSize: '12px', fontWeight: 600, cursor: 'pointer', background: pasoModal === idx + 1 ? '#3B82F6' : pasoModal > idx + 1 ? '#DBEAFE' : '#F3F4F6', color: pasoModal === idx + 1 ? '#FFF' : pasoModal > idx + 1 ? '#3B82F6' : '#9CA3AF' }}>{idx + 1}. {label}{pasoModal > idx + 1 ? ' ✓' : ''}</button>
                ))}
              </div>
            </div>
            ...
            <div className="modal-body" style={{ padding: '20px 24px' }}>
              ...
              {pasoModal === 3 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  {DESTINOS_CON_CAMA.includes(destino) && (<div><label className="label">Asignar cama</label><input type="text" value={camaAsignada} onChange={e => setCamaAsignada(e.target.value)} placeholder={`Ej: ${destino === 'INTERNAMIENTO' ? 'Cuarto 101' : destino === 'OBSERVACION' ? 'Obs 3' : destino === 'HOSPITALIZACION' ? 'Hab 201' : 'UCI 5'}`} className="input-modern" /></div>)}
